...
class EmailNotifier(Notifier):
    ...
    def __init__(self, email_server, **kwargs):
        ...
        # Pass remaining values to the next class
        super().__init__(**kwargs)

    def send(self, message):
        logs = super().send(message)

        # Add email log at the beginning
        logs.insert(
            0,
...
class HybridAlertChannel(EmailNotifier, SMSNotifier):
    ...
    def __init__(self, sender_id, email_server, sms_gateway):
        ...
        # Pass all values through the inheritance chain
        super().__init__(
    ...
    def send(self, message):
        ...
        # Add hybrid message at the beginning
        logs.insert(
            0,
            "[HYBRID ALERT] Initiating dual channels..."
        )
...
# Create hybrid notification object
...
for class_name in HybridAlertChannel.__mro__:
    print(class_name)
